...
設計確定 2026-09-07。実装は Phase 0 から順に進行中。
このファイルは実装が進むたびに「状態」欄を更新すること。
...
| board 5本 | 35日前 | 昨日（Phase 3 後） | 過去日ナビ・カレンダードットを Phase 3 まで守る |
...
| Phase | 内容 | 対象 | 状態 |
|---|---|---|---|
| 0 | 検出スニペットで sf_bookings の健全性確認（date無し0件の確認・重複・日曜ext・連日ペア）と掃除 | クラウドデータ | **進行中** |
| 1 | persistentLocalCache 移植 | salon 3本 + notify 3本 | 未着手 |
| 2 | 日付窓（暫定広窓）。パッチスクリプト sf_readwindow_20260907.py（アンカー出現1回検証・2フェーズ書き込み・バックアップ・node --check・期待差分の機械算出） | 12本全部 | 未着手 |
| — | **Firebase 使用状況を実測**し Phase 3・4 の要否を数字で判断 | — | 未着手 |
| 3 | 過去日ナビ＋カレンダードットのオンデマンド読み → board の窓を昨日へ | board 5本 | 未着手 |
