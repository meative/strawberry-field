...
| 2 | 日付窓（暫定広窓）。パッチスクリプト sf_readwindow_20260908.py（アンカー出現1回検証・2フェーズ書き込み・バックアップ・node --check・期待差分の機械算出）。timely は fromBoard 削除パスの窓ガードも同時に投入 | 12本全部 | **実装済み 2026-09-08**（Playwright 12/12：SF_WINDOW_START がモードどおり・JS エラー0） |
| — | **Firebase 使用状況を実測**し Phase 3・4 の要否を数字で判断 | — | Phase 2 反映後 |
